...
from routers.items import router, hotspot_router, avis_router

app = FastAPI(title="Agrégateur Touristique Madagascar", version="2.0.0")

# Montage des fichiers statiques (depuis le dossier app)
...
async def startup():
    """Vérifier la connexion à Elasticsearch au démarrage"""
...
async def home(request: Request):
    return templates.TemplateResponse(
        request=request, name="index.html", context={"data": "a"}
    )